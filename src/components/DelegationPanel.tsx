...
export function DelegationPanel(props: {
  ...
  return (
    ...
    >
      ...
      <Show when={!props.task.coordinatorMode && props.task.stagedNotification}>
        <details>
          <summary>
            Child updates ({props.task.stagedNotification?.notificationIds.length ?? 0}) — ready for
            review
          </summary>
            ...
            {(message) => (
              ...
              >
                ...
                <p style={{ 'white-space': 'pre-wrap', 'overflow-wrap': 'anywhere' }}>
                  {preview() === message.deliveryId ? message.prompt : message.prompt.slice(0, 180)}
                </p>
      ...
      <Show
        when={
          project()?.allowAgentTaskCreation &&
              ...
              <Show
                ...
                fallback={
                  <small>
                    ...
                    resumable conversation. Delegate task… remains available.
                  </small>
                }
              >
                <button disabled={busy()} onClick={() => void act(() => restartForTools(agent.id))}>
