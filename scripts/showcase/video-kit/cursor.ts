 ...
 */
export const installCursor = async (page: Page, options: CursorOptions = {}): Promise<void> => {
  await page.addInitScript(
    (args) => {
      const attach = (): void => {
        if (document.getElementById('vk-cursor')) return;
        const isArrow = args.style === 'arrow';
        const size = args.size ?? Math.min(64, Math.max(28, Math.round(window.innerWidth / 40)));
...
export const setCursorVisible = async (page: Page, isVisible: boolean): Promise<void> => {
  await page.evaluate((args) => document.body.classList.toggle(args.cls, !args.isVisible), {
    cls: HIDDEN_CLASS,
    isVisible,
  });
};
...
const centerOf = async (target: Locator): Promise<Point> => {
  ...
  const isOutside =
    frame && (center.x < 0 || center.y < 0 || center.x > frame.width || center.y > frame.height);
  if (isOutside) {
    throw new Error(
...
export const createPointer = (page: Page): Pointer => {
  ...
  const moveTo = async (to: Point, move: { durationMs: number; arc?: number }): Promise<void> => {
    if (!position) {
  ...
  const glideTo: Pointer['glideTo'] = async (target, options = {}) => {
    ...
    await moveTo({ x: (box?.x ?? to.x) - 40, y: to.y }, { durationMs: durationMs * 0.65 });
