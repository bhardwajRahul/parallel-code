...
**Copied from Super Productivity** (`e2e/video-kit/` at `a33c409e`), without its tests.
Local changes: Prettier formatting and one lint suppression in `recorder.ts`. Fix a
primitive upstream first when you can, then copy it here, so the two copies stay easy to diff.

**Rule:** nothing in this folder imports app code. Project specifics (seed data, selectors,
copy, brand logos, variants) live in the consumer; here that is
[`../electron-app.ts`](../electron-app.ts) and [`../scenes/`](../scenes/). That keeps the
kit liftable into another project or a workspace package.
...
const camera = createCamera(page, '#root'); // stage = the element covering the viewport
...
The behavior tests live upstream in Super Productivity (`npm run video:kit-test` there):
layer ownership, paused clock, navigation cover and failed setup cleanup, camera framing,
pointer timing, tap ripples, and recording/trim pairing. Change a primitive there, with a
test, and copy it here.
