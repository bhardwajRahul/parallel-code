...
export const readRecordingTrim = (src: string, variant: string): RecordingTrim => {
  ...
  try {
    ...
    throw new Error(`Missing or unreadable trim metadata: ${sidecarPath}. Recapture the video.`);
  }
  if (
 ...
 */
export const encodeGif = async (input: EncodeInput & { width?: number }): Promise<void> => {
  const { src, out, trim } = input;
  const scale = `scale=${input.width ?? 1024}:-1:flags=lanczos`;
  const palette = path.join(path.dirname(out), `.palette-${path.basename(out, '.gif')}.png`);
...
export const writeContactSheet = async (input: {
  ...
  const frames = [...new Set(input.atSeconds.map((s) => Math.max(0, Math.round(s * OUTPUT_FPS))))];
